...
const AddRoom = ({ fetchRoom }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try{
            const response = await axios.post(`${process.env.REACT_APP_SERVER_ADDRESS}/room/add`, formData);
            // handle success response
            setMessages(`<font color="green">Room added Successfully: ${response.data.room.room_number}</font>` );
            setFormData({ name: '', roomNumber: '', bedInfo: '' });
            fetchRoom(); // Refresh the room list
        } catch (error) {
            setMessages(`<font color="red">Error adding room: ${error.response.data.message || error.message}</font>`);
        }
    };
