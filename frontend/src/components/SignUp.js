...
export const SignUpPage = () => {
    ...
    const apiServer = process.env.REACT_APP_SERVER_ADDRESS;
    ...
    const registerUser = async (e) => {
        ...
        if (formData.passwordHash !== confirmPasswordValue) {
            setErrorMessage('Passwords do not match');
            return;
        }

        try {
            const response = await axios.post(`${apiServer}/signup`, formData);
            setSuccessMessage(`User added Successfully: ${response.data.user.email}`);
            setFormData({ email: '', passwordHash: '', firstName: '', lastName: '' });
        } catch (error) {
            setErrorMessage(`Error adding User: ${error.response.data.message || error.message}`);
        }
    };
